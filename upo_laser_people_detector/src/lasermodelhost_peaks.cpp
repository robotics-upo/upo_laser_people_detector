...
public:
	...
	LaserModelPeaks(rclcpp::Logger&& log, Ort::Env& env, std::string const& modelPath, float near, float far, float thresh, float person_radius, bool use_cuda = true) :
...
class LaserModelHostPeaks : public rclcpp::Node {
	...
	void onLaserScan(sensor_msgs::msg::LaserScan const& msg)
	{
...

		{
			...
			for (size_t i = 0; i < people.size(); i ++) {
				auto& p = people[i];
				out->markers.emplace_back(makePersonMarker(msg.header.frame_id, 1+i, p[1], p[2], 0.4f, 1.0));
			}
...
public:
		...
		Node{"lasermodelhostpeaks", options}
	{
		...
		m_model.emplace(get_logger(), m_ortEnv, model_file, near, far, thresh, radius, use_cuda);
